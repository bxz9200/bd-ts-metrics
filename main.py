...
def parse_command_line_arguments():
    ...
    parser.add_argument("--rdp","--real_data_path", default=None, help="Real TS data path (.cvs file)")
    parser.add_argument("--sdp","--synthetic_data_path", default=None, help="Synthetic TS data path (.npy file)")
    parser.add_argument("--sql","--seq_len", default=100, help="TS data sequence length")
...
def main():
    ...
    # load real data
    if args.rdp.endswith(".csv"):
        data = load_my_own_data_csv(args.rdp, int(args.sql))
        # data = load_my_own_data_csv_v2(args.rdp, int(args.sql))
    elif args.rdp.endswith(".npy"):
        data = load_my_own_data_numpy(args.rdp)
    else:
        raise Exception("Real data: wrong data format")


    # load generated data
    if args.sdp.endswith(".csv"):
        generated_data = load_my_own_data_csv(args.sdp, int(args.sql))
        # generated_data = load_my_own_data_csv_v2(args.sdp, int(args.sql))
    elif args.sdp.endswith(".npy"):
        generated_data = load_my_own_data_numpy(args.sdp)
    else:
